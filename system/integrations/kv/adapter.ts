...
const kv = useMock ? (mockKv as any) : realKv;
...
export class KVAdapter {
  ...
  async get<T = unknown>(key: string): Promise<T | null> {
    try {
      const result = await kv.get(this.key(key));
  ...
  async getList<T extends ListItem>(
    ...
    try {
      ...
      const raw = await kv.get(this.key(key));
      if (!raw) return [];

      const items = (raw as string[]).map(item => JSON.parse(item) as T);
  ...
  async addToList<T extends ListItem>(
    ...
    try {
      const raw = (await kv.get(this.key(key)) as string[]) || [];
