...
/**
 * Trend Report - Generates robust app ideas from multi-source trends
 */
...
async function main() {
  ...
  const multi = top.filter(t => (t.sources || []).length >= 2);
  ...
  const report = buildReport(selected, concepts, selectionPool);
  console.log(report);
}
...
function buildReport(trend, concepts, topTrends) {
  ...
  const risks = [
    'Trend might be transient — validate with 2–3 weeks of signal.',
    'Risk of being a thin wrapper — need real workflow ownership.',
    'Data access quality can limit automation accuracy.'
  ].join('\n');
